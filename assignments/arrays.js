...
for (let i = 0; i < inventory.length; i++) {
  if (inventory[i].id === 33) {
    console.log(
      `Car 33 is a ${inventory[i].car_make} ${inventory[i].car_model}`
    );
  }
}

let answer = inventory.find(item => item.id === 33);
console.log(`Car 33 is a ${answer.car_make} ${answer.car_model}`);
...
for (let i = 0; i < inventory.length; i++) {
  if (inventory[i].id === inventory.length) {
    lastCar = [inventory[i].car_make, inventory[i].car_model];
  }
}
console.log(lastCar);
...
//Best way
carModelsSorted = inventory.map(item => item.car_model).sort();
console.log(carModelsSorted);
// ==== Challenge 4 ====
...
// best way
carYears = inventory.map(item => item.car_year);
console.log(carYears);
...
// Best way
oldCars = carYears.filter(item => item < 2000);
console.log(oldCars.length);
